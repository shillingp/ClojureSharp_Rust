...
fn convert_abstract_syntax_tree_to_code(syntax_tree_node: &SyntaxTreeNode) -> String {
    match syntax_tree_node.type_ {
        ...
        SyntaxTreeNodeType::EqualityCheck => convert_expression_syntax_tree_node_to_code(syntax_tree_node),
...
fn convert_branch_syntax_tree_node_to_code(syntax_tree_node: &SyntaxTreeNode) -> String {
    ...
    if syntax_tree_node.value.clone().unwrap() == "if" {
        index_offset = 1;
        output.push_str("(if ");
        output.push_str(&convert_abstract_syntax_tree_to_code(&syntax_tree_node.children[0]));
        output.push('\n');
    }

    if syntax_tree_node.children.len() == index_offset + 1 {
        output.push_str(convert_abstract_syntax_tree_to_code(&syntax_tree_node.children[index_offset]).as_str());
    } else{
        output.push_str("(do \n");
        output.push_str(syntax_tree_node.children.iter().skip(index_offset)
            .map(convert_abstract_syntax_tree_to_code)
            .collect::<Vec<String>>()
            .join("\n").as_str());
        output.push(')');
